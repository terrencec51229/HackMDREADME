...
# Migrate On-premises Workloads To AWS - General Machine
...
Besides everything is from scratch on the cloud, the most quickest/easiest way is to have a copy there. Therefore,  not only the AWS-native services but also several 3^rd^ party solutions are able to make it happen and worth to keep an eye on as well. So, let's get started.
