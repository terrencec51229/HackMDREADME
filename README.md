...
<span class="fontColor">The other thing is that SR supports ECMP natively.</span> If we only care about the destination (XR6) only instead of what the preferred path is then the result will be changed. Both XR2 and XR3 are not only just valid but also best to forward the traffic due to they have the same length forwarding path.
